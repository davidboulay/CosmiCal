...
// Pick how much of the event's text fits given its rendered height. As an
// event gets shorter we degrade gracefully: full title + time at normal size →
// title + time at a smaller size → title only. Heights here track HOUR_HEIGHT
// (72px): e.g. a 30-min event is 36px tall, a 50-min one is 60px.
function displayModeFor(durationMinutes: number): WeekEventDisplayMode {
  if (durationMinutes < 30) return "xs" // title only
  if (durationMinutes < 50) return "sm" // title + time, smaller font
  if (durationMinutes < 90) return "md" // title + time, normal font
  return "lg" // title (up to 2 lines) + time
}
